...
class chiron_core_db {
		...
		public function sources_get_all(){
	    	...
	    	$result = $chiron_db->query($query) or print('Query failed: ' . $this->connection->error);
	    	$return = array();
		...
		public function sources_get_least_updated($number = 5){
		    ...
		    $query_result = $chiron_db->query($query) or print('Query failed: '.$chiron_db->connection->error);
			$return = array();
		    while($source_array = $chiron_db->fetch_array($result)) {
				$source_object = new chiron_source();
				$source_object->load($source_array);
		...
		public function sources_get_some_by_ids($ids_sources){
		    ...
		    $query_result = $chiron_db->query($query) or print('Query failed: '.$chiron_db->connection->error." QUERY [".$query."]");
			$return = array();
		    while($source_array = $chiron_db->fetch_array($result)) {
				$source_object = new chiron_source();
				$source_object->load($source_array);
		...
		public function items_get_by_day($day){
		    ...
		    $result = $chiron_db->query($query) or print('Query failed: ' . $chiron_db->connection->error);  
		 	$return = array();
