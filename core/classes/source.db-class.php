...
class chiron_source_db{
	...
	public function source_add($title, $type, $url){
	    ...
	    $result = $chiron_db->query($query) or print("Query failed: ".$chiron_db->connection->error." Query:".$query);;
		return $result;
	}
	...
	public function source_update($id, $title, $url){
		...
		return $chiron_db->query($query) or print("Query failed: ".$chiron_db->connection->error." Query:".$query);
	}
