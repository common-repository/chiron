...
class chiron_db {
	...
	public function __construct($db_srv, $db_usr, $db_pwd, $db_dbs, $db_pre) {
		$this->connection = new mysqli($db_srv, $db_usr, $db_pwd);
    	$this->connection->select_db( $db_dbs) or die('Could not select database');
		$this->prefix = $db_pre;
		$this->load_schema();
	}
	...
	public function query($query){
		$result = $this->connection->query($query) or print('Query failed: ' . $this->connection->error);
		$this->result = $result;
		return $result;
	}
